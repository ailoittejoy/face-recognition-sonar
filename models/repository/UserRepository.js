...
exports.checkAndLogin = async (body) => {
	try {
		...
		return { success: true, message: 'Login successful', data: modifyOutputData(user) };
	} catch (error) {
...
exports.checkAndCreateUser = async (body) => {
	try {
		...
		const hashedPassword = await bcrypt.hash(body.password, bcrypt.genSaltSync(8)); // 10 is the salt rounds
